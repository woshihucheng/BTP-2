...
void load_images(const string & dir, vector< Mat > & img_lst, const cv::Size &sz);
void sample_neg(const vector< Mat > & full_neg_lst, vector< Mat > & neg_lst, const Size & size);
Mat get_hogdescriptor_visu(const Mat& color_origImg, vector<float>& descriptorValues, const Size & size);
void compute_hog(const vector< Mat > & img_lst, vector< Mat > & gradient_lst, const Size & size);
void train_svm(const vector< Mat > & gradient_lst, const vector< int > & labels, const string &name);
...
int main(int argc, char** argv)
{
	...
	load_images(pos_dir, pos_lst, sz);
	labels.assign(pos_lst.size(), +1);
	const unsigned int old = (unsigned int)labels.size();
	load_images(neg_dir, neg_lst, sz);
	//sample_neg(full_neg_lst, neg_lst, sz);
	labels.insert(labels.end(), neg_lst.size(), -1);

	compute_hog(pos_lst, gradient_lst,sz);
	compute_hog(neg_lst, gradient_lst, sz);
	
	string name = "my_hog_"+to_string(sz.width)+"_"+ to_string(sz.height) + ".yml";
	
	train_svm(gradient_lst, labels, name);

	//test_it(sz); // change with your parameters
