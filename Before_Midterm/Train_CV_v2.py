...
Additional Change:
-Directory and seed as input
-Search trained file for given params
'''
import cv2
import numpy as np
import svm
...
dr = input("Enter Training Directory")
...
test_writing_dir = input("Enter  the directory in which you want the generated test images")+"Test\\"
i = 0
...
exist=os.path.isfile('SEED_'+str(SEED)+'.model')
m=0
if exist:
    m=svmutil.svm_load_model('SEED_'+str(SEED)+'.model')
    ...
    svmutil.svm_save_model('SEED_'+str(SEED)+'.model',m)
    k, b, c = svmutil.svm_predict(y_tst, x_tst, m, '-b 1')
    correct = 0
    with open("log_test_results_" + str(SEED) + ".txt", "w") as fwrite2:
        ...
        fwrite2.write("Test Accuracy: " + str(correct / len(y_tst)) + "\n")
